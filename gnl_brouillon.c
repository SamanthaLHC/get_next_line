...
char *get_one_line_fd(int fd)
{
	...
	i = 1;
	finish = 0;
	// /!\ gerer le cas ou il y a deja des \n dans save/!\
	// ca sert a rien de lire s il y  a deja le \n a traiter
	//strcpy de save? a chaque appel de gnl
	line = save;
	...
	while (finish == 0)
	{
		...
		if (tmp != NULL)
			ft_strcpy(line, tmp);
		// avant de read je dois traiter le contenu de save transfere
		//dans line
